...
ggplot(data = gapminder_2007, 
       mapping = aes(x = continent, 
                     y = lifeExp ,
                     fill=continent)) +
  geom_boxplot(outlier.shape = NA) +  #https://stackoverflow.com/questions/5677885/ignore-outliers-in-ggplot2-boxplot
...
Column and Bar function difference:

geom_bar:

\- does not require a y- axis,it counts the number of cases in each group –\> useful, when plotting frequencies and counts

-counts number of cases in each group

geom_col:

-requires x- and y-axis, because it maps the height of the bar to the value of the data

-expects already summarized data

Summary: geom_bar automatically in a dataset like gapminder_2007 counts the number of observations for each continent, while geom_col uses a summarized dataset like gapminder_summary_2007, where the count was already calculated
...
=defining a sequence of bin edges for the histogram from 40 to 85 with an interval of 2.5 –\> the bins start at 40, then 42.5, 45, 47.5, and so on, up to 85. Each bin includes values from its left edge up to but not including the right edge.
...
1.  typing country_colors into the R console and pressing Enter–\>list where each country name is associated with a specific color value

2.  typing ?country_colors into the R console –\> leads to a "help page"

    --\>stores the colors for the 142 countries
